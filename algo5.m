...
mask = imread("Dataset/Training-Dataset/Masks-Ideal/4_A_hgr2B_id06_1.bmp");
...
%% --------------------------
% Eliminar lo que no son dedos
stats = regionprops(fingersMask, {'Area', 'ConvexArea', 'Eccentricity', 'EquivDiameter', 'EulerNumber', 'MajorAxisLength', 'MinorAxisLength', 'Orientation', 'Perimeter'});
%%
sumFingers = 0;
if ~isempty(stats)

    for i=1:length(stats)
        pred = trainedModel.predictFcn(struct2table(stats(i)));        
